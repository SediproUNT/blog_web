                        ...
                        @foreach ($posts as $post)
                            <div class="col-md-6">
                                <div class="card mb-4">
                                    ...
                                    <div class="card-body">
                                        <!-- Contenido breve del post -->
                                        <p class="card-text">{{ Str::limit($post->body, 100) }}</p>
                                        <!-- Imagen del post (versión pequeña) si está disponible -->
                                        @if ($post->image_url)
                                            <img src="{{ $post->image_url }}" alt="Post Image" class="img-fluid img-thumbnail mb-3" data-bs-toggle="modal" data-bs-target="#postModal{{ $post->id }}">
                                        @endif
                                        <!-- Botón para abrir el modal -->
                                        <a href="#" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#postModal{{ $post->id }}">Leer más</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
